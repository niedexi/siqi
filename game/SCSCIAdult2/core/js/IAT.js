  ...
  for (var i = 0; i < 4; i++) {
    ...
    for (var j = 0; j < numrounds; j++) {
      ...
      if (stype == "ci") {
        //Pick genius
        if (picker == 0) {
          if (geniusCount < roundType.ten) {
            round.category = template.cat2.datalabel;
            geniusCount++;
          } else if (diligentCount < roundType.seven) {
        ...
        //Pick diligent
          ...
          if (diligentCount < roundType.seven) {
            round.category = template.cat1.datalabel;
            diligentCount++;
          } else if (geniusCount < roundType.ten) {
            round.category = template.cat2.datalabel;
            geniusCount++;
          } else {
            round.category = template.catA.datalabel;
      ...
      if (stype == "ic") {
        ...
        //Pick genius
          ...
          if (geniusCount < roundType.seven) {
            round.category = template.cat2.datalabel;
            geniusCount++;
          } else if (diligentCount < roundType.ten) {
            round.category = template.cat1.datalabel;
            diligentCount++;
          } else {
            round.category = template.catA.datalabel;
...
function writeFile() {
  ...
  let final = JSON.stringify({
    category: "SCSCIAdult2",
    folderID: "1233_R4I-cnvdODvE6BHq60zbnnRg9s6U",
    name: ID,
    data: dataArray,
  });
